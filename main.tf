...
resource "aws_ssm_parameter" "docdb_url_catalogue" {
  ...
  type  = "String"
#  The below value we have to take from docdb cluster ( Connect to this cluster with an application ), some value need to be refer like username and password from above code
